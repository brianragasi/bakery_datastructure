...
if (isset($_GET['delete_product'])) {
    $productIdToDelete = (int)$_GET['delete_product'];

    if ($product->deleteProduct($productIdToDelete)) {
        header("Location: ../views/admin/manage_products.php?success=Product deleted successfully.");
        exit;
    } else {
        $error = $product->getError();
        header("Location: ../views/admin/manage_products.php?error=" . urlencode("Error deleting product: " . $error));
        exit;
    }
}
...
if (isset($_GET['toggle_featured'])) {
    $productId = $adminProduct->escapeString($_GET['toggle_featured']);
    ...
    header("Location: ../views/admin/manage_products.php?message=" . urlencode($message));
    exit;
}
